...
@RestController
@RequestMapping("/api/cases/{caseId}/genogram")
@RequiredArgsConstructor
public class GenogramController {

    private final GenogramService genogramService;
    ...
    private void checkCaseAccessOrThrow(UUID caseId, UUID userId) {
        ...
        if (!isMember && !hasSupervision) {
            throw new ResponseStatusException(
                    ...
                    "이 사례의 가계도를 조회할 권한이 없습니다."
            );
        }
    }

    @PostMapping("/render")
    public ResponseEntity<GenogramRenderResponse> renderGenogram(
            @PathVariable("caseId") UUID caseId
    ) {
        UUID userId = currentUser.getUserIdOrThrow();
        checkCaseAccessOrThrow(caseId, userId);

        String url = genogramService.renderGenogramAndGetUrl(caseId);
